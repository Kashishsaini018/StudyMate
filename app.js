...
function renderPracticeMyBuilder(){const h=getHistory(),counts={incorrect:0,silly:0,skipped:0,topics:0,notebook:0};h.forEach(r=>r.questions.forEach(q=>{if(q.status==='Incorrect')counts.incorrect++;if(q.silly)counts.silly++;if(q.status==='Skipped')counts.skipped++;if(q.topic)counts.topics++;if(q.mistakeNotebook)counts.notebook++}));$('practiceMyBuilder').innerHTML=`<div class="field"><label><input id="pmIncorrect" type="checkbox"> Incorrect Questions (${counts.incorrect})</label><label><input id="pmSilly" type="checkbox"> Silly Mistakes (${counts.silly})</label><label><input id="pmSkipped" type="checkbox"> Skipped Questions (${counts.skipped})</label><label><input id="pmTopics" type="checkbox"> Weak Topics (${counts.topics})</label><label><input id="pmNotebook" type="checkbox"> Saved in Mistake Notebook (${counts.notebook})</label></div><div class="field"><label>Number of questions</label><select id="pmCount"><option>10</option><option>20</option><option>30</option><option>45</option></select></div><div class="small-muted">Practice attempts are kept separate and never change the original test result.</div><div class="actions"><button id="startMyPractice" class="primary">Start Practice →</button></div>`;$('startMyPractice').onclick=startMyPractice}
async function startMyPractice(){
 const h=getHistory(),pool=[];
 const want={i:$('pmIncorrect').checked,s:$('pmSilly').checked,k:$('pmSkipped').checked,t:$('pmTopics').checked,n:$('pmNotebook')?.checked};
 if(!Object.values(want).some(Boolean))return alert('Choose at least one category.');
 for(const r of h){
   for(const q of (r.questions||[])){
     if((want.i&&q.status==='Incorrect')||(want.s&&q.silly)||(want.k&&q.status==='Skipped')||(want.t&&q.topic)||(want.n&&q.mistakeNotebook))pool.push({record:r,original:q});
   }
 }
...
function renderDaySummary(){
  const all=getDaySummaries(),today=localDayKey(),d=all[today]||{subjects:{},time:'',reflection:''},subs=d.subjects||{};
  $('daySummaryContent').innerHTML=`<form id="daySummaryForm" class="panel day-summary-form"><h3>📋 Today's Study by Subject</h3><div class="day-subject-grid">${SUBJECTS.map(s=>{const x=subs[s]||{};return `<article class="day-subject-card"><div class="day-subject-title"><span>${s==='Physics'?'⚡':s==='Chemistry'?'🧪':s==='Botany'?'🌿':'🧬'}</span><strong>${s}</strong></div><div class="field"><label>What did you study?</label><textarea rows="3" data-ds-study="${s}" placeholder="Chapters/topics studied...">${escapeHtml(x.study||'')}</textarea></div><div class="field"><label>Questions practiced</label><input type="number" min="0" data-ds-q="${s}" value="${x.questions||0}"></div></article>`}).join('')}</div><div class="two-col"><div class="field"><label>⏱️ Effective study time</label><input id="dsTime" value="${escapeHtml(d.time||'')}" placeholder="e.g. 6h 25m"></div><div class="field"><label>💭 How was your day? <span class="small-muted">(optional)</span></label><input id="dsReflection" value="${escapeHtml(d.reflection||'')}" placeholder="Short reflection..."></div></div><div class="actions"><button class="primary">Save Today's Summary ✓</button></div></form><div class="report-card"><h3>Recent Days</h3><div class="day-summary-history">${Object.entries(all).sort((a,b)=>b[0].localeCompare(a[0])).slice(0,14).map(([date,x])=>{const total=SUBJECTS.reduce((a,s)=>a+Number(x.subjects?.[s]?.questions||0),0);return `<div class="day-summary-row"><strong>${new Date(date+'T00:00:00').toLocaleDateString(undefined,{day:'numeric',month:'short',year:'numeric'})}</strong><span>${total} Q</span><span>${escapeHtml(x.time||'—')}</span></div>`}).join('')||'<div class="empty">Save your first day summary.</div>'}</div></div>`;
  $('daySummaryForm').onsubmit=e=>{e.preventDefault();const subjects={};SUBJECTS.forEach(s=>subjects[s]={study:document.querySelector(`[data-ds-study="${CSS.escape(s)}"]`).value.trim(),questions:Number(document.querySelector(`[data-ds-q="${CSS.escape(s)}"]`).value||0)});all[today]={subjects,time:$('dsTime').value.trim(),reflection:$('dsReflection').value.trim(),savedAt:new Date().toISOString()};saveDaySummaries(all);renderDaySummary();toast('Day summary saved ✓')};
}

/* ===== Mistake Notebook ===== */
async function renderMistakeNotebook(){
 const groups={Physics:[],Chemistry:[],Botany:[],Zoology:[]};getHistory().forEach(r=>(r.questions||[]).forEach(q=>{if(!q.mistakeNotebook)return;const s=subjectFor(q.number,r.questions.length)||q.subject||'Other';if(groups[s])groups[s].push({record:r,q})}));
 const total=Object.values(groups).reduce((a,v)=>a+v.length,0);if(!total){$('mistakeNotebookContent').innerHTML='<div class="empty">No questions have been added yet. Open a test result → Questions and tick the questions you want to keep in your Mistake Notebook.</div>';return}
 let html='<div class="mistake-subject-tabs">'+SUBJECTS.map((s,i)=>`<button class="mistake-subject-tab ${i===0?'active':''}" data-mistake-sub="${s}">${s} <span>${groups[s].length}</span></button>`).join('')+'</div><div id="mistakeSubjectPanel"></div><div class="mistake-practice-cta"><h3>Ready to fix your mistakes?</h3><p>Practice the questions you deliberately saved from your test results.</p><button class="primary" id="notebookPracticeBtn">🔥 Practice My Mistakes</button></div>';$('mistakeNotebookContent').innerHTML=html;
 const renderSubject=(subject)=>{const items=groups[subject]||[];$('mistakeSubjectPanel').innerHTML=`<section class="report-card"><div class="page-head"><div><span class="eyebrow">${subject.toUpperCase()}</span><h3>${subject} Mistakes</h3></div><span class="pill">${items.length} saved</span></div>${items.length?`<div class="mistake-list">${items.map((it,i)=>`<article class="mistake-entry"><div class="mistake-entry-head"><div><strong>Q${it.q.number}</strong><span>${escapeHtml(it.record.name)}</span></div><div class="mistake-tags"><span class="badge incorrect">${it.q.silly?'Silly Mistake':'Incorrect'}</span>${it.q.topic?`<span class="topic-pill">${escapeHtml(it.q.topic)}</span>`:'<span class="topic-pill">Topic not recorded</span>'}</div></div><div class="mistake-question-viewer" id="mistakeViewer_${subject}_${i}"><div class="viewer-placeholder">Loading exact PDF question…</div></div></article>`).join('')}</div>`:'<div class="empty">No saved mistakes in ${subject}.</div>'}</section>`;items.forEach((it,i)=>renderMistakeEntryPdf(it,$(`mistakeViewer_${subject}_${i}`)))};
 renderSubject(SUBJECTS[0]);document.querySelectorAll('[data-mistake-sub]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-mistake-sub]').forEach(x=>x.classList.toggle('active',x===b));renderSubject(b.dataset.mistakeSub)});$('notebookPracticeBtn').onclick=()=>openModal('practiceMyModal',renderPracticeMyBuilder);
}
...
function showScreen(name){
  ...
  if(name==='nextTest') renderNextTestPlanner();
...
function getNeetTarget(){
  const saved=localStorage.getItem('studymate_neet_target_local');
  const parsed=localDateTimeValueToMs(saved);
  if(Number.isFinite(parsed)) return parsed;
  return localDateTimeValueToMs('2027-05-02T14:00');
}
function renderCountdown(){
  ...
  const note=$('neetCountdownNote');if(note)note.textContent='Target: 2 May 2027 • 2:00 PM (editable in Settings)';
}
...
function renderNcertFocus(){
  const data=getNcert();
  const subjects=['Botany','Zoology'];
  const chapterProgress=(subject,chapter)=>{const x=data[`${subject}::${chapter}`]||{};const done=NCERT_STEPS.filter(([k])=>x[k]).length;return {done,pct:Math.round(done/NCERT_STEPS.length*100),x}};
  $('ncertContent').innerHTML=`<div class="ncert-zone-grid">${subjects.map(subject=>`<section class="report-card ncert-subject-card"><div class="ncert-subject-head"><div><span class="eyebrow">${subject.toUpperCase()}</span><h3>📖 ${subject} NCERT Zone</h3></div><strong>${Math.round(SYLLABUS[subject].reduce((a,c)=>a+chapterProgress(subject,c).pct,0)/SYLLABUS[subject].length)}%</strong></div><div class="barline"><i style="width:${Math.round(SYLLABUS[subject].reduce((a,c)=>a+chapterProgress(subject,c).pct,0)/SYLLABUS[subject].length)}%"></i></div><div class="ncert-chapter-list">${SYLLABUS[subject].map((chapter,i)=>{const p=chapterProgress(subject,chapter);return `<article class="ncert-chapter-card"><div><b>${i+1}. ${escapeHtml(chapter)}</b><small>${p.done}/${NCERT_STEPS.length} milestones • ${p.pct}% complete</small></div><div class="barline"><i style="width:${p.pct}%"></i></div><button class="secondary" data-ncert-open="${escapeHtml(subject)}::${escapeHtml(chapter)}">Open</button></article>`}).join('')}</div></section>`).join('')}</div>`;
  document.querySelectorAll('[data-ncert-open]').forEach(b=>b.onclick=()=>{const [subject,chapter]=b.dataset.ncertOpen.split('::');renderNcertChapter(subject,chapter)});
}
function renderNcertChapter(subject,chapter){
  const data=getNcert(),key=`${subject}::${chapter}`,x=data[key]||{};
  $('ncertContent').innerHTML=`<div class="report-card"><div class="page-head"><div><span class="eyebrow">${subject.toUpperCase()}</span><h3>📖 ${escapeHtml(chapter)}</h3></div><button class="secondary" id="ncertBack">← All Chapters</button></div><div class="ncert-progress-big"><strong>${Math.round(NCERT_STEPS.filter(([k])=>x[k]).length/NCERT_STEPS.length*100)}%</strong><div class="barline"><i style="width:${Math.round(NCERT_STEPS.filter(([k])=>x[k]).length/NCERT_STEPS.length*100)}%"></i></div></div>${NCERT_STEPS.map(([k,l])=>`<label class="checkline"><input type="checkbox" data-ncert-step="${k}" ${x[k]?'checked':''}> ${l}</label>`).join('')}</div>`;
  $('ncertBack').onclick=renderNcertFocus;
  document.querySelectorAll('[data-ncert-step]').forEach(c=>c.onchange=e=>{const d=getNcert(),z=d[key]||{};z[e.target.dataset.ncertStep]=e.target.checked;d[key]=z;saveNcert(d);renderNcertChapter(subject,chapter);toast('NCERT progress saved ✓')});
}
...
function renderDailyGoals(){
  const goals=getGoals(),today=localDayKey(),g=goals[today]||{studyHours:0,studyMinutes:0,ncertChapter:'',questionGoals:{},testDateTime:''};
  const summary=getDaySummaries()[today]||{};
  const qActual=summary.subjects||{};
  const totalActual=SUBJECTS.reduce((a,s)=>a+Number(qActual[s]?.questions||0),0);
  const studyActual=parseStudyMinutes(summary.time);
  const qg=g.questionGoals||{};
  $('goalsContent').innerHTML=`<form id="goalsForm" class="panel"><div class="two-col"><div class="field"><label>⏱️ Study goal — Hours</label><input id="goalHours" type="number" min="0" max="24" value="${g.studyHours||0}"></div><div class="field"><label>Minutes</label><input id="goalStudyMinutes" type="number" min="0" max="59" value="${g.studyMinutes||0}"></div><div class="field"><label>📖 NCERT goal — Chapter (Botany/Zoology)</label><select id="goalNcertChapter"><option value="">Select a chapter</option>${['Botany','Zoology'].flatMap(s=>SYLLABUS[s].map(c=>`<option value="${escapeHtml(s+'::'+c)}" ${g.ncertChapter===s+'::'+c?'selected':''}>${s} — ${escapeHtml(c)}</option>`)).join('')}</select></div></div><div class="report-card"><h3>❓ Question Practice Goals</h3><div class="goal-subject-grid">${SUBJECTS.map(s=>`<div class="goal-subject-card"><strong>${s}</strong><select data-goal-chapter="${s}"><option value="">Select chapter</option>${SYLLABUS[s].map(c=>`<option value="${escapeHtml(c)}" ${qg[s]?.chapter===c?'selected':''}>${escapeHtml(c)}</option>`).join('')}</select><input data-goal-q="${s}" type="number" min="0" placeholder="No. of questions" value="${qg[s]?.count||''}"></div>`).join('')}</div></div><div class="field"><label>📝 Test goal — Day & Time</label><input id="goalTestDateTime" type="datetime-local" value="${escapeHtml(g.testDateTime||'')}"></div><button class="primary">Save Today's Goals ✓</button></form><div class="report-card"><h3>📈 Today's Progress</h3>${goalProgressRow('Effective study',studyActual,(Number(g.studyHours||0)*60+Number(g.studyMinutes||0)),'min')}<div class="goal-row"><div><b>Questions</b><span>${totalActual} Q</span></div><div class="barline"><i style="width:${Math.min(100,totalQuestionGoal(g)?totalActual/totalQuestionGoal(g)*100:0)}%"></i></div></div><div class="small-muted">Question goal: ${totalQuestionGoal(g)||0} across selected chapters.</div></div>`;
  document.querySelectorAll('[data-goal-chapter]').forEach(sel=>sel.onchange=()=>{});
  $('goalsForm').onsubmit=e=>{e.preventDefault();const questionGoals={};SUBJECTS.forEach(s=>{const chapter=document.querySelector(`[data-goal-chapter="${CSS.escape(s)}"]`).value;const count=Number(document.querySelector(`[data-goal-q="${CSS.escape(s)}"]`).value||0);if(chapter&&count>0)questionGoals[s]={chapter,count}});goals[today]={studyHours:Number($('goalHours').value||0),studyMinutes:Number($('goalStudyMinutes').value||0),ncertChapter:$('goalNcertChapter').value,questionGoals,testDateTime:$('goalTestDateTime').value};saveGoals(goals);renderDailyGoals();toast('Daily goals saved ✓')};
}
function totalQuestionGoal(g){return Object.values(g.questionGoals||{}).reduce((a,x)=>a+Number(x.count||0),0)}
function goalProgressRow(label,actual,goal,unit){const pct=goal?Math.min(100,actual/goal*100):0;return `<div class="goal-row"><div><b>${label}</b><span>${actual}${unit?' '+unit:''} / ${goal||'—'}</span></div><div class="barline"><i style="width:${pct}%"></i></div></div>`}

function localDayKey(){const d=new Date(),p=n=>String(n).padStart(2,'0');return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}`}
...
const REVISION_SESSION_KEY='studymate_revision_sessions_v1';
function getRevisionSessions(){try{return JSON.parse(localStorage.getItem(REVISION_SESSION_KEY)||'{}')}catch{return{}}}
function saveRevisionSessions(x){localStorage.setItem(REVISION_SESSION_KEY,JSON.stringify(x))}
function renderIntelligentRevision(){
  const p=getPlanner(),today=new Date(),rows=[];
  for(const [key,x] of Object.entries(p)){const [s,c]=key.split('::');if(!x||x.mastered)continue;const last=x.lastStudy?new Date(x.lastStudy):null;let due=!last,label='Start revision';if(last){const days=Math.floor((today-last)/86400000);if(days>=30&&!x.rev5){due=true;label='Rev-5 due';}else if(days>=21&&!x.rev4){due=true;label='Rev-4 due';}else if(days>=7&&!x.rev3){due=true;label='Rev-3 due';}else if(days>=3&&!x.rev2){due=true;label='Rev-2 due';}else if(days>=1&&!x.rev1){due=true;label='Rev-1 due';}}if(due)rows.push({s,c,label})}
  const sessions=getRevisionSessions();
  $('revisionContent').innerHTML=`<div class="report-card revision-hero"><span class="eyebrow">SMART REVISION</span><h3>🧠 Revision is now actionable</h3><p>Each task opens a short revision session instead of only showing a button. Finish the checklist and StudyMate records the session.</p></div><div class="revision-task-list">${rows.length?rows.slice(0,30).map((x,i)=>{const id=encodeURIComponent(x.s+'::'+x.c),done=sessions[id]?.done;return `<article class="revision-task-card ${done?'done':''}"><div><span class="eyebrow">${escapeHtml(x.s)}</span><h3>${escapeHtml(x.c)}</h3><p>${escapeHtml(x.label)}</p></div><button class="primary" data-start-revision="${id}">${done?'Review Again':'Start Revision →'}</button></article>`}).join(''):'<div class="empty">No revision task is due. Update your Success Planner milestones and StudyMate will generate the next revision automatically.</div>'}</div>`;
  document.querySelectorAll('[data-start-revision]').forEach(b=>b.onclick=()=>startRevisionSession(decodeURIComponent(b.dataset.startRevision)));
}
function startRevisionSession(key){const [subject,chapter]=key.split('::');const sessions=getRevisionSessions(),saved=sessions[encodeURIComponent(key)]||{};const tasks=[['notes','Review notes / formulas / diagrams'],['ncert','Read the relevant NCERT section'],['pyq','Practice 10–20 PYQs or module questions'],['mistakes','Review your mistakes for this chapter'],['recall','Do a 5-minute active recall without notes']];$('revisionContent').innerHTML=`<div class="report-card revision-session"><div class="page-head"><div><span class="eyebrow">${escapeHtml(subject)}</span><h2>${escapeHtml(chapter)}</h2></div><button class="secondary" id="revisionBack">← Revision Queue</button></div><div class="revision-session-list">${tasks.map(([k,l])=>`<label class="revision-check"><input type="checkbox" data-rev-session="${k}" ${saved[k]?'checked':''}><span>${l}</span></label>`).join('')}</div><div class="actions"><button class="primary" id="finishRevision">Finish Revision ✓</button></div></div>`;$('revisionBack').onclick=renderIntelligentRevision;$('finishRevision').onclick=()=>{const x={};document.querySelectorAll('[data-rev-session]').forEach(c=>x[c.dataset.revSession]=c.checked);x.done=tasks.every(([k])=>x[k]);x.date=new Date().toISOString();sessions[encodeURIComponent(key)]=x;saveRevisionSessions(sessions);const p=getPlanner(),item=p[key]||{};item.lastStudy=new Date().toISOString();p[key]=item;savePlanner(p);toast(x.done?'Revision completed 🎉':'Revision session saved');renderIntelligentRevision()}}
...
function renderSmartWeakness(){
  ...
  $('weaknessContent').innerHTML=`<div class="weakness-hero"><div><span class="eyebrow">DATA-DRIVEN</span><h3>🔥 Smart Weakness Engine</h3><p>Only topics with <b>5 or more recorded mistakes</b> appear, so the list stays focused.</p></div><div class="weakness-fire">🔥</div></div><div class="weakness-cards">${rows.length?rows.map((x,i)=>{const level=x.score>=45?'HIGH':x.score>=25?'MEDIUM':'WATCH';return `<article class="weakness-card"><div class="weakness-card-top"><div><span class="rank-badge">${i+1}</span><div><h3>${escapeHtml(x.topic)}</h3><p>${x.wrong} mistakes • ${x.silly} silly • ${x.skipped} skipped • ${x.days}d since practice</p></div></div><span class="priority-badge ${level.toLowerCase()}">${level}</span></div><div class="weakness-meter"><span style="width:${Math.min(100,x.score)}%"></span></div><div class="weakness-stats"><b>${x.acc.toFixed(0)}%<small>accuracy</small></b><b>${x.wrong}<small>mistakes</small></b><b>${x.score.toFixed(0)}<small>priority</small></b></div></article>`}).join(''):'<div class="empty">No topic has reached 5 recorded mistakes yet. Keep recording topics during test analysis.</div>'}</div>`;
}
...
/* ===== V5.2 change pack ===== */
function subjectForRecordQuestion(record,q){return subjectFor(q.number,(record.questions||[]).length)||q.subject||'Other'}

// Result page: opt-in Mistake Notebook checkbox + exact PDF question review.
function resultPanelQuestions(record){
  const incorrect=record.questions.filter(q=>q.status==='Incorrect');
  const skipped=record.questions.filter(q=>q.status==='Skipped');
  const rows=incorrect.length?incorrect.map((q,i)=>{const s=subjectForRecordQuestion(record,q);return `<article class="result-question-card">
    <div class="result-question-head"><div><strong>Q${q.number}</strong><div class="small-muted">${escapeHtml(s)}${q.topic?' • '+escapeHtml(q.topic):''}</div></div>
    <label class="save-mistake-check"><input type="checkbox" data-save-mistake="${i}" ${q.mistakeNotebook?'checked':''}> <span>Save to Mistake Notebook</span></label></div>
    <div class="result-question-viewer" id="resultPdfViewer_${record.id}_${i}"><div class="viewer-placeholder">Loading exact PDF question…</div></div>
    <div class="result-question-meta"><span>❌ Incorrect</span>${q.silly?'<span>😵 Silly Mistake</span>':''}${q.reason?`<span>Reason: ${escapeHtml(q.reason)}</span>`:''}</div>
  </article>`}).join(''):'<div class="empty">🎉 No incorrect questions in this test.</div>';
  const skippedNote=skipped.length?`<div class="quote-card" style="margin-top:14px">⏭️ ${skipped.length} skipped question${skipped.length===1?'':'s'} are not added to the Mistake Notebook unless you later choose another practice category.</div>`:'';
  return `<section class="report-card result-section" data-section="questions"><h3>❌ Incorrect Questions</h3><p class="small-muted">Tick only the questions you want to keep in your Mistake Notebook. Each question below uses the original saved PDF.</p><div class="result-pdf-question-list">${rows}</div>${skippedNote}</section>`;
}
const __renderResultV51=renderResult;
renderResult=function(record){
  __renderResultV51(record);
  document.querySelectorAll('[data-save-mistake]').forEach(cb=>cb.onchange=()=>{
    const idx=Number(cb.dataset.saveMistake), q=record.questions.filter(x=>x.status==='Incorrect')[idx];
    if(!q)return;
    q.mistakeNotebook=cb.checked;
    const h=getHistory(),pos=h.findIndex(x=>x.id===record.id);
    if(pos>=0){h[pos]=record;localStorage.setItem(LS.history,JSON.stringify(h.slice(0,100)));}
    toast(cb.checked?'Added to Mistake Notebook ✓':'Removed from Mistake Notebook');
  });
};

// Success Planner: one chapter card at a time with Previous / Next controls.
const PLANNER_INDEX_KEY='studymate_planner_index_v2';
function getPlannerIndex(subject){return Math.max(0,Math.min(SYLLABUS[subject].length-1,Number(localStorage.getItem(`${PLANNER_INDEX_KEY}_${subject}`)||0)))}
function setPlannerIndex(subject,i){localStorage.setItem(`${PLANNER_INDEX_KEY}_${subject}`,String(Math.max(0,Math.min(SYLLABUS[subject].length-1,i))))}
function renderSuccessPlanner(){
  const data=getPlanner();let total=0,done=0;
  SUBJECTS.forEach(s=>SYLLABUS[s].forEach(c=>{const st=plannerChapterStatus(data,s,c);total+=st.total;done+=st.checked}));
  const progress=total?Math.round(done/total*100):0;
  $('plannerContent').innerHTML=`<div class="planner-hero report-card"><div class="planner-hero-copy"><span class="eyebrow">PLAN • STUDY • REVISE • MASTER</span><h3>NEET Success Planner</h3><p>Work through one chapter card at a time. Your milestone checks are saved automatically.</p></div><div class="planner-progress"><strong>${progress}%</strong><span>overall</span></div></div><div class="planner-stats report-card"><div><b>${done}</b><span>Milestones done</span></div><div><b>${total-done}</b><span>Remaining</span></div><div><b>${SUBJECTS.reduce((a,s)=>a+SYLLABUS[s].length,0)}</b><span>Chapters</span></div></div><div class="planner-tabs">${SUBJECTS.map((s,i)=>`<button type="button" class="planner-tab ${i===0?'active':''}" data-planner-sub="${s}">${s}</button>`).join('')}</div><div id="plannerPanel"></div>`;
  const renderSubject=(subject)=>{
    document.querySelectorAll('[data-planner-sub]').forEach(b=>b.classList.toggle('active',b.dataset.plannerSub===subject));
    const chapters=SYLLABUS[subject],idx=getPlannerIndex(subject),chapter=chapters[idx],key=`${subject}::${chapter}`,x=data[key]||{},st=plannerChapterStatus(data,subject,chapter);
    const mobileSteps=PLANNER_STEPS.map(step=>`<label class="planner-mobile-step ${x[step.key]?'checked':''}"><span><b>${escapeHtml(step.label)}</b>${step.sub?`<small>${escapeHtml(step.sub)}</small>`:''}</span><input type="checkbox" data-planner-key="${escapeHtml(key)}" data-planner-step="${step.key}" ${x[step.key]?'checked':''}><i>${x[step.key]?'✓':''}</i></label>`).join('');
    $('plannerPanel').innerHTML=`<section class="report-card planner-single-card"><div class="planner-single-top"><div><span class="planner-number-badge">${idx+1}/${chapters.length}</span><span class="eyebrow">${subject.toUpperCase()}</span><h3>${escapeHtml(chapter)}</h3><p>${st.checked}/${st.total} milestones completed • ${st.pct}% complete</p></div><span class="planner-mobile-status">${st.mastered?'🏆 Mastered':st.status==='On Track'?'🟢 On Track':st.checked?'🟡 Needs Work':'🔴 Not Started'}</span></div><div class="planner-mobile-progress"><span style="width:${st.pct}%"></span></div><div class="planner-mobile-steps planner-single-steps">${mobileSteps}</div><div class="planner-chapter-nav"><button class="secondary" id="plannerPrev" ${idx===0?'disabled':''}>← Previous</button><span>Chapter ${idx+1} of ${chapters.length}</span><button class="primary" id="plannerNext" ${idx===chapters.length-1?'disabled':''}>Next →</button></div></section>`;
    document.querySelectorAll('[data-planner-key]').forEach(cb=>cb.onchange=()=>{const all=getPlanner(),item=all[cb.dataset.plannerKey]||{};item[cb.dataset.plannerStep]=cb.checked;if(cb.checked)item.lastStudy=new Date().toISOString();all[cb.dataset.plannerKey]=item;savePlanner(all);renderSuccessPlanner();requestAnimationFrame(()=>document.querySelector(`[data-planner-sub="${CSS.escape(subject)}"]`)?.click());toast(cb.checked?'Milestone completed ✓':'Milestone unchecked')});
    $('plannerPrev').onclick=()=>{setPlannerIndex(subject,idx-1);renderSubject(subject)};
    $('plannerNext').onclick=()=>{setPlannerIndex(subject,idx+1);renderSubject(subject)};
  };
  renderSubject('Physics');
  document.querySelectorAll('[data-planner-sub]').forEach(btn=>btn.onclick=()=>renderSubject(btn.dataset.plannerSub));
}

// Day Summary helper uses local date and the new subject-wise structure; legacy summaries remain readable.

// Next Test Syllabus & Planner. Weightage is an indicative trend layer, not an official NTA prediction.
const NEXT_TEST_KEY='studymate_next_test_planner_v1';
const NEXT_TEST_HIGH={
  Physics:['Laws of Motion','Work, Energy, and Power','System of Particles and Rotational Motion','Gravitation','Thermodynamics','Electric Charges and Fields','Electrostatic Potential and Capacitance','Current Electricity','Moving Charges and Magnetism','Electromagnetic Induction','Ray Optics and Optical Instruments','Dual Nature of Radiation and Matter','Semiconductor Electronics: Materials, Devices, and Simple Circuits'],
  Chemistry:['Some Basic Concepts of Chemistry','Chemical Bonding and Molecular Structure','Chemical Thermodynamics','Equilibrium','Solutions','Electrochemistry','Chemical Kinetics','d- and f-Block Elements','Coordination Compounds','Some Basic Principles of Organic Chemistry (GOC)','Hydrocarbons','Aldehydes, Ketones, and Carboxylic Acids','Organic Compounds Containing Nitrogen (Amines)'],
  Botany:['Morphology of Flowering Plants','Cell: The Unit of Life','Cell Cycle and Cell Division','Photosynthesis in Higher Plants','Plant Growth and Development','Sexual Reproduction in Flowering Plants','Principles of Inheritance and Variation','Molecular Basis of Inheritance','Biotechnology: Principles and Processes','Biotechnology and its Applications','Ecosystem','Biodiversity and Conservation'],
  Zoology:['Animal Kingdom','Biomolecules','Breathing and Exchange of Gases','Body Fluids and Circulation','Excretory Products and their Elimination','Neural Control and Coordination','Chemical Coordination and Integration','Human Reproduction','Reproductive Health','Evolution','Human Health and Disease']
};
const NEXT_TEST_DIFF={Physics:'High effort / numerical',Chemistry:'Moderate / mixed',Botany:'NCERT-heavy',Zoology:'NCERT + concept'};
function getNextTest(){try{return JSON.parse(localStorage.getItem(NEXT_TEST_KEY)||'{}')}catch{return{}}}
function saveNextTest(x){localStorage.setItem(NEXT_TEST_KEY,JSON.stringify(x))}
function nextTestWeight(subject,chapter){return (NEXT_TEST_HIGH[subject]||[]).includes(chapter)?2:1}
function renderNextTestPlanner(){
  const saved=getNextTest(),dt=saved.dateTime||'',difficulty=saved.difficulty||'Moderate',sel=saved.syllabus||{};
  const selectedCount=SUBJECTS.reduce((a,s)=>a+(sel[s]?.length||0),0);
  $('nextTestContent').innerHTML=`<form id="nextTestForm" class="panel"><div class="two-col"><div class="field"><label>📝 Next Test Name</label><input id="ntName" value="${escapeHtml(saved.name||'')}" placeholder="e.g. Weekly Test 05"></div><div class="field"><label>🕐 Test Day & Time</label><input id="ntDateTime" type="datetime-local" value="${escapeHtml(dt)}" required></div><div class="field"><label>Difficulty Level</label><select id="ntDifficulty"><option ${difficulty==='Easy'?'selected':''}>Easy</option><option ${difficulty==='Moderate'?'selected':''}>Moderate</option><option ${difficulty==='Hard'?'selected':''}>Hard</option></select></div></div><div class="next-test-info"><b>Remaining time</b><strong id="ntRemaining">Enter the test date and time.</strong><small>StudyMate uses recent NEET trend data to prioritize higher-frequency chapters. This is an indicative planning aid, not an official NTA weightage.</small></div><h3>📚 Select the syllabus for the next test</h3>${SUBJECTS.map(subject=>`<section class="next-test-subject"><div class="next-test-subject-head"><h4>${subject}</h4><span>${NEXT_TEST_DIFF[subject]}</span></div><div class="next-test-chapters">${SYLLABUS[subject].map((c,i)=>{const checked=(sel[subject]||[]).includes(c);return `<label class="next-test-chapter ${checked?'checked':''}"><input type="checkbox" data-nt-sub="${subject}" value="${escapeHtml(c)}" ${checked?'checked':''}><span><b>${i+1}. ${escapeHtml(c)}</b><small>${nextTestWeight(subject,c)===2?'Higher recent trend priority':'Standard trend priority'}</small></span></label>`}).join('')}</div></section>`).join('')}<div class="actions"><button class="primary">Save & Build Revision Plan →</button></div></form><div id="nextTestPlan"></div>`;
  const updateRemaining=()=>{const ms=localDateTimeValueToMs($('ntDateTime').value);const el=$('ntRemaining');if(!Number.isFinite(ms)){el.textContent='Enter the test date and time.';return}const d=ms-Date.now();if(d<=0){el.textContent='The test time has passed.';return}const days=Math.floor(d/86400000),hours=Math.floor(d%86400000/3600000),mins=Math.floor(d%3600000/60000);el.textContent=`${days} days ${hours}h ${mins}m remaining`;};
  $('ntDateTime').oninput=updateRemaining;updateRemaining();
  $('nextTestForm').onsubmit=e=>{e.preventDefault();const syllabus={};SUBJECTS.forEach(s=>{syllabus[s]=[...document.querySelectorAll(`[data-nt-sub="${CSS.escape(s)}"]:checked`)].map(x=>x.value)});if(!Object.values(syllabus).some(v=>v.length))return toast('Select at least one chapter.');const x={name:$('ntName').value.trim()||'Next Test',dateTime:$('ntDateTime').value,difficulty:$('ntDifficulty').value,syllabus,savedAt:new Date().toISOString()};saveNextTest(x);renderNextTestPlanner();renderNextTestPlan(x);toast('Next test revision plan built ✓')};
  if(saved.dateTime&&selectedCount)renderNextTestPlan(saved);
}
function renderNextTestPlan(plan){
  const remaining=Math.max(0,localDateTimeValueToMs(plan.dateTime)-Date.now()),hours=Math.max(1,remaining/3600000);const diff=plan.difficulty||'Moderate';
  const diffMult=diff==='Hard'?1.35:diff==='Easy'?.8:1;
  const all=[];SUBJECTS.forEach(s=>(plan.syllabus?.[s]||[]).forEach(c=>all.push({s,c,w:nextTestWeight(s,c)})));const totalW=all.reduce((a,x)=>a+x.w,0);const tasks=[];
  all.forEach(x=>{let alloc=Math.max(0.25,(hours*Math.min(0.18,0.05+x.w*0.025)*diffMult)*(x.w/2));alloc=Math.min(3,alloc);tasks.push({...x,alloc})});
  const totalAllocated=tasks.reduce((a,x)=>a+x.alloc,0),scale=Math.min(1,hours/Math.max(1,totalAllocated));
  const planHtml=tasks.map(x=>{const mins=Math.max(15,Math.round(x.alloc*scale*60/5)*5),task=x.w===2?'High-priority revision: revise notes/NCERT, then solve PYQs and review mistakes.':'Core revision: revise notes/NCERT and solve a focused PYQ/module set.';return `<article class="next-test-task"><div><span class="subject-dot">${x.s[0]}</span><h4>${escapeHtml(x.c)}</h4><p>${task}</p></div><strong>${mins} min</strong></article>`}).join('');
  const intensity=diff==='Hard'?'Add a second PYQ/error-review pass for your highest-weight chapters.':diff==='Easy'?'Use the saved time for active recall and a short mixed test.':'Balance revision with PYQs and mistake review.';
  $('nextTestPlan').innerHTML=`<div class="report-card next-test-plan"><div class="next-test-plan-head"><div><span class="eyebrow">REVISION PLAN</span><h3>🎯 ${escapeHtml(plan.name)}</h3><p>${escapeHtml(intensity)}</p></div><span class="pill">${all.length} chapters</span></div><div class="next-test-tasks">${planHtml}</div><div class="quote-card">📌 Trend basis: recent 2021–2026 chapter-frequency analyses. Chapter weightage varies by year; use this plan to allocate revision time, not to skip low-frequency chapters.</div></div>`;
}
