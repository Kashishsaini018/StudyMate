...
function renderPracticeResult(result){
 ...
 const reviewRows=review.map(q=>{const ua=q.userAnswer==null?'—':(q.questionType==='numeric'?String(q.userAnswer):String.fromCharCode(65+q.userAnswer));const ca=q.correctAnswer==null?'':(q.questionType==='numeric'?String(q.correctAnswer):String.fromCharCode(65+q.correctAnswer));return `<div class="question-item"><div><b>Q${q.number}</b><small>${escapeHtml(q.s||'Practice')} • ${escapeHtml(q.c||'')}</small><small>${q.status==='Skipped'?'Skipped':`Your answer: ${escapeHtml(ua)}${q.hasKey?` • Correct: ${escapeHtml(ca)}`:''}`}</small></div><span class="badge ${q.status==='Incorrect'?'incorrect':'skipped'}">${q.status}</span></div>`}).join('');
