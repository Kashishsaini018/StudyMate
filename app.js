...
async function makeBackup(){
 ...
 try{
  ...
  const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`StudyMate_Backup_${new Date().toISOString().slice(0,10)}.smbackup`;document.body.appendChild(a);a.click();setTimeout(()=>{a.remove();URL.revokeObjectURL(url)},1500);
...
function renderSuccessPlanner(){
  ...
  const renderSubject=(subject,chapterIndex=0)=>{
    ...
    const chapters=SYLLABUS[subject],i=Math.max(0,Math.min(chapterIndex,chapters.length-1)),chapter=chapters[i];
    ...
    $('plannerPanel').innerHTML=`<div class="report-card planner-table-card"><div class="planner-table-head"><div><h3>${escapeHtml(subject)}</h3><span class="small-muted">Chapter ${i+1} of ${chapters.length} • ${subject==='Botany'||subject==='Zoology'?'NCERT tracking included.':'Use NCERT Zone only for Biology.'}</span></div><span class="planner-legend">${st.pct}% complete</span></div><article class="planner-mobile-card ${st.status.toLowerCase().replace(' ','')}"><div class="planner-mobile-card-head"><div><span class="planner-number-badge">${i+1}</span><strong>${escapeHtml(chapter)}</strong><small>${st.checked}/${st.total} milestones completed</small></div><span class="planner-mobile-status">${st.mastered?'🏆 Mastered':st.status==='On Track'?'🟢 On Track':st.checked?'🟡 Needs Work':'🔴 Not Started'}</span></div><div class="planner-mobile-progress"><span style="width:${st.pct}%"></span></div><div class="planner-mobile-steps">${mobileSteps}</div></article><div class="planner-chapter-nav"><button class="secondary" id="plannerPrev" ${i===0?'disabled':''}>← Previous Chapter</button><span class="planner-chapter-count">${i+1} / ${chapters.length}</span><button class="secondary" id="plannerNext" ${i===chapters.length-1?'disabled':''}>Next Chapter →</button></div></div>`;
    bindPlannerChecks(subject,i);
    $('plannerPrev').onclick=()=>renderSubject(subject,i-1);$('plannerNext').onclick=()=>renderSubject(subject,i+1);
  };
  function bindPlannerChecks(subject,chapterIndex){document.querySelectorAll('[data-planner-key]').forEach(cb=>cb.onchange=()=>{const all=getPlanner(),item=all[cb.dataset.plannerKey]||{};item[cb.dataset.plannerStep]=cb.checked;if(cb.checked)item.lastStudy=new Date().toISOString();all[cb.dataset.plannerKey]=item;savePlanner(all);renderSuccessPlanner();requestAnimationFrame(()=>{document.querySelector(`[data-planner-sub="${CSS.escape(subject)}"]`)?.click();setTimeout(()=>renderSubject(subject,chapterIndex),0)});toast(cb.checked?'Milestone completed ✓':'Milestone unchecked');});}
...
function renderNcertFocus(){
  const data=getNcert();const subjects=['Botany','Zoology'];
  const steps=[['reading','NCERT Reading'],['lines','Important Lines Reviewed'],['rev1','First Revision'],['rev2','Second Revision'],['pyq','PYQs Practiced']];
  let total=0,done=0;
  subjects.forEach(s=>SYLLABUS[s].forEach(c=>{total+=steps.length;const x=data[`${s}::${c}`]||{};done+=steps.filter(st=>x[st[0]]).length;}));
  const pct=total?Math.round(done/total*100):0;
  const subjectCards=subjects.map((s,si)=>{const sp=SYLLABUS[s];const subjectDone=sp.reduce((n,c)=>n+steps.filter(st=>(data[`${s}::${c}`]||{})[st[0]]).length,0),subjectTotal=sp.length*steps.length,subjectPct=Math.round(subjectDone/subjectTotal*100);return `<div class="ncert-subject-card"><div class="ncert-card-head"><div><span class="eyebrow">${s.toUpperCase()}</span><h3>${s} NCERT Zone</h3></div><strong>${subjectPct}%</strong></div><div class="barline"><i style="width:${subjectPct}%"></i></div><div class="ncert-chapter-list">${sp.map((c,i)=>{const x=data[`${s}::${c}`]||{},cp=Math.round(steps.filter(st=>x[st[0]]).length/steps.length*100);return `<button class="ncert-chapter" data-ncert-open="${escapeHtml(s)}::${escapeHtml(c)}"><span><b>${i+1}. ${escapeHtml(c)}</b><small>${cp}% complete</small></span><span>${cp}% →</span></button>`}).join('')}</div></div>`}).join('');
  $('ncertContent').innerHTML=`<div class="ncert-hero report-card"><div><span class="eyebrow">BIOLOGY ONLY • BOTANY + ZOOLOGY</span><h3>NCERT Zone</h3><p>Track reading, important lines, revisions and PYQs chapter by chapter.</p></div><div class="ncert-total-progress"><strong>${pct}%</strong><span>complete</span></div></div>${subjectCards}<div id="ncertChapterPanel"></div>`;
  document.querySelectorAll('[data-ncert-open]').forEach(b=>b.onclick=()=>{const [s,c]=b.dataset.ncertOpen.split('::');const key=`${s}::${c}`,x=data[key]||{};$('ncertChapterPanel').innerHTML=`<div class="report-card"><div class="modal-head"><div><span class="eyebrow">${escapeHtml(s)}</span><h3>${escapeHtml(c)}</h3></div><strong>${Math.round(steps.filter(st=>x[st[0]]).length/steps.length*100)}%</strong></div>${steps.map(([k,l])=>`<label class="checkline"><input type="checkbox" data-ncert-step="${k}" ${x[k]?'checked':''}> ${l}</label>`).join('')}<div class="actions"><button class="secondary" id="ncertCloseChapter">Close</button></div></div>`;$('ncertCloseChapter').onclick=()=>$('ncertChapterPanel').innerHTML='';document.querySelectorAll('[data-ncert-step]').forEach(cbx=>cbx.onchange=e=>{const d=getNcert(),z=d[key]||{};z[e.target.dataset.ncertStep]=e.target.checked;d[key]=z;saveNcert(d);renderNcertFocus();setTimeout(()=>document.querySelector(`[data-ncert-open="${CSS.escape(key)}"]`)?.click(),0);toast('NCERT progress saved ✓')});});
}
...
function renderIntelligentRevision(){
  ...
  h.forEach(r=>(r.questions||[]).forEach(q=>{if(q.status==='Correct'&&!q.silly)return;pool.push({record:r,q});}));
  const seen=new Set(),unique=[];for(const item of pool){const k=`${item.record.id}:${item.q.number}`;if(!seen.has(k)){seen.add(k);unique.push(item)}}
  const due=unique.slice(0,30);
  const dueRows=due.map((x,i)=>`<div class="focus-row"><div><b>Q${x.q.number} • ${escapeHtml(x.record.name)}</b><small>${escapeHtml(x.q.topic||'Topic not recorded')} • Previous: ${escapeHtml(x.q.status)}${x.q.silly?' • Silly mistake':''}</small></div><span class="badge incorrect">Revision</span></div>`).join('');
  $('revisionContent').innerHTML=`<div class="revision-hero report-card"><div><span class="eyebrow">SMART REVISION</span><h3>Revision Queue</h3><p>StudyMate builds a revision set from your own analysed tests. It uses the original PDF question cutouts when the source PDF is saved.</p></div><div class="revision-count"><strong>${unique.length}</strong><span>questions ready</span></div></div><div class="report-card"><h3>🧠 Recommended Revision</h3>${dueRows||'<div class="empty">Analyse a test and record topics/statuses to create a revision queue.</div>'}<div class="actions"><button class="primary" id="generateRevisionQuestions" ${due.length?'':'disabled'}>Generate Revision Questions</button></div></div>`;
  $('generateRevisionQuestions')?.addEventListener('click',()=>generateRevisionQuestions(due));
}
async function generateRevisionQuestions(items){
  const chosen=items.slice(0,20),questions=[];
  for(const item of chosen){let pdf=null,info=null;if(item.record.hasPdf){try{const blob=await getPdfBlob(item.record.id);if(blob&&pdfjsLib){pdf=await pdfjsLib.getDocument({data:await blob.arrayBuffer()}).promise;const map=await extractPdfMap(pdf);info=map.get(Number(item.q.number));}}catch(e){console.warn('Revision PDF load failed',e)}}questions.push({key:`${item.record.id}:${item.q.number}`,number:item.q.number,pdf,info,recordName:item.record.name,previous:item.q,status:null,recordId:item.record.id});}
  state.practiceQuiz={title:'Revision from My Tests',questions,index:0,answers:{},historyMode:true};showScreen('practice');await renderHistoryPracticeQuiz();
}
